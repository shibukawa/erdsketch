...
export function ModelingWorkspacePage({ initialInvitationToken, initialParticipantRecovery }: ModelingWorkspacePageProps) {
  ...
  if (workspaceMode === "dfd") {
    return <>
      ...
      {exportDialog && <ExportDialog canonicalProjectJSON={exportDialog.snapshot} projectName={activeProject?.displayName ?? "ERDSketch project"} initialNameMode={nameDisplayMode} initialCardDisplayMode={exportDialog.cardDisplayMode} initialCrudOrientation={dfd.crudMatrix?.orientation ?? "processes_rows"} onClose={closeExportDialog} />}
