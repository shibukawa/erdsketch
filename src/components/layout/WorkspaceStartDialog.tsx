  ...
  return <div className="modal modal-open" role="dialog" aria-modal="true" aria-labelledby="workspace-start-title">
    <div className="modal-box flex max-h-[92vh] max-w-6xl flex-col overflow-hidden rounded-2xl bg-slate-50 p-0 shadow-2xl">
      ...
      <div className="min-h-0 flex-1 space-y-7 overflow-y-auto p-6">
        ...
        <section><h3 className="text-lg font-bold">Open another project</h3><p className="mt-1 text-sm text-slate-600">Upload a portable archive or JSON project file. Direct local access is available only after you choose a file or folder.</p><div className="mt-3 flex flex-wrap gap-3"><button type="button" className="btn btn-outline gap-2" disabled={disabled} onClick={handleUploadClick}><HardDriveUpload size={17} />Upload project file</button><input ref={uploadRef} type="file" accept=".json,.gz,.erdsketch,.txtar.gz,application/json,application/gzip" className="hidden" onChange={handleUpload} /><button type="button" className="btn btn-outline gap-2" disabled={disabled || !localFileAvailable} onClick={handleOpenLocal}><FolderOpen size={17} />Open from local storage</button><button type="button" className="btn btn-ghost gap-2" disabled={busy} onClick={handleManage}><FolderCog size={17} />Manage projects</button></div>{!localFileAvailable && <p className="mt-3 text-xs text-slate-500">Direct file-system access is unavailable here. Upload still works, and browsers cannot scan local files without your permission.</p>}</section>
      </div>
