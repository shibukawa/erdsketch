...
export function OriginPrivateStoragePanel({ projects, activeProjectId, disabled, run, onCreate, onSaveAs, onLoad, onRename, onDelete, onExport, onImport, onClose }: Props) {
  ...
  return <div className="grid min-h-0 flex-1 gap-6 overflow-y-auto p-6 md:grid-cols-[minmax(0,1fr)_minmax(0,1.3fr)] md:overflow-hidden" role="tabpanel">
    <section>
      ...
      <div className="mt-4 rounded-lg border border-slate-200 p-4"><h3 className="font-bold">Portable archive</h3><p className="mt-1 text-sm text-slate-600">Move a project between browsers or keep a user-visible backup.</p><div className="mt-3 flex gap-2"><button type="button" className="btn btn-outline btn-sm gap-2" disabled={disabled} onClick={handleImportClick}><Upload size={16} />Import</button><button type="button" className="btn btn-outline btn-sm gap-2" disabled={disabled} onClick={handleExport}><Download size={16} />Export</button></div><input ref={importRef} type="file" accept=".json,.gz,.erdsketch,.txtar.gz,application/json,application/gzip" className="hidden" onChange={handleImport} /></div>
    </section>
