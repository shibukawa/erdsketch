...
export function ProjectCanvasSelectorDialog({ erdCanvases, dfdCanvases, active, required = false, onSelect, onCreate, onRename, onClose }: Props) {
  ...
  const renderSection = (kind: ProjectCanvasKind, label: string) => {
    ...
    return <section className="min-w-0 p-5"><div className="mb-3 flex items-center gap-2"><Icon size={18} className={kind === "erd" ? "text-amber-700" : "text-blue-700"} /><h3 className="font-bold">{label}</h3><span className="badge badge-ghost badge-sm">{canvasesByKind[kind].length}</span></div><div className="space-y-2">{canvasesByKind[kind].map((canvas) => <div key={canvas.id}>{editing?.kind === kind && editing.id === canvas.id ? <form className="flex min-w-0 gap-2 rounded-lg border border-blue-300 bg-blue-50 p-2" onSubmit={handleRename}><input autoFocus className="input input-bordered input-sm min-w-0 flex-1" value={renameDraft} onChange={handleRenameDraft} /><button className="btn btn-primary btn-sm btn-square" disabled={saving}><Check size={15} /></button></form> : <div className="relative"><button type="button" data-kind={kind} data-canvas-id={canvas.id} className={`flex min-h-14 w-full items-center gap-3 rounded-lg border px-4 py-2 pr-14 text-left transition ${active.kind === kind && active.id === canvas.id ? "border-blue-300 bg-blue-50 hover:bg-blue-100" : "border-slate-200 hover:border-blue-300 hover:bg-slate-50"}`} onClick={handleSelect}><span data-i18n-skip className="line-clamp-2 min-w-0 flex-1 font-bold leading-tight">{canvas.name}</span>{active.kind === kind && active.id === canvas.id && <span className="badge badge-primary badge-sm shrink-0">Current</span>}</button><button type="button" data-kind={kind} data-canvas-id={canvas.id} className="btn btn-ghost btn-sm btn-square absolute right-2 top-1/2 -translate-y-1/2" onClick={handleStartRename} aria-label={`Rename ${canvas.name}`}><Pencil size={14} /></button></div>}</div>)}</div><form data-kind={kind} className="mt-4 flex gap-2" onSubmit={handleCreate}><input data-kind={kind} className="input input-bordered input-sm min-w-0 flex-1" placeholder={`New ${kind.toUpperCase()} canvas`} value={newNames[kind]} onChange={handleNewName} /><button className="btn btn-primary btn-sm gap-1" disabled={saving || !newNames[kind].trim()}><Plus size={15} />Create</button></form></section>;
  };

  return <div className="modal modal-open" role="dialog" aria-modal="true" aria-labelledby="project-canvas-selector-title" data-ai-assistant-disabled><div className="modal-box max-w-4xl rounded-xl bg-white p-0"><header className="flex items-center justify-between border-b border-slate-200 px-6 py-4"><div><p className="text-xs font-bold uppercase tracking-wide text-slate-500">{required ? "Choose a workspace" : "Project navigation"}</p><h2 id="project-canvas-selector-title" className="text-xl font-bold">ERD &amp; DFD canvases</h2>{required && <p className="mt-1 text-sm text-slate-600">The project is ready. Select the diagram you want to open.</p>}</div>{!required && <button type="button" className="btn btn-ghost btn-sm btn-square" onClick={onClose} aria-label="Close canvas selector"><X size={18} /></button>}</header><div className="grid divide-y divide-slate-200 md:grid-cols-2 md:divide-x md:divide-y-0">{renderSection("erd", "Entity relationship diagrams")}{renderSection("dfd", "Data flow diagrams")}</div></div>{!required && <button className="modal-backdrop" onClick={onClose} aria-label="Close canvas selector" />}</div>;
}
