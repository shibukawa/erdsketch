  ...
  return <div data-tour="model-catalog" className="modal modal-open" role="dialog" aria-modal="true" aria-labelledby="model-catalog-title">
    ...
    <div className="modal-box h-[78vh] max-w-6xl rounded-xl bg-white p-0">
      ...
      <div className="h-[calc(78vh-126px)] overflow-auto px-6 py-4"><table className="table table-pin-rows"><thead><tr><th>Model</th><th>Role</th><th>Owner canvas</th><th>Placed canvases</th><th className="text-right">Actions</th></tr></thead><tbody>{visibleSeeds.map((seed) => {
        ...
        return <tr key={seed.id}><td><span data-i18n-skip className="font-bold">{seed.title}</span>{dfdOnly && <span className="badge badge-warning badge-sm ml-2">DFD only</span>}<span className="block text-xs text-slate-500">{modelPlacements.length} placement{modelPlacements.length === 1 ? "" : "s"}</span></td><td><span className="badge badge-outline">{seed.role}</span></td><td>{owner ? <span data-i18n-skip>{canvases.find((canvas) => canvas.id === owner.canvasId)?.name}</span> : <span className="text-slate-400">Shared / none</span>}</td><td><div className="flex max-w-md flex-wrap gap-1">{modelPlacements.map((placement) => <button key={placement.canvasId} type="button" data-canvas-id={placement.canvasId} data-seed-id={seed.id} className={`badge gap-1 ${placement.accessMode === "owner" ? "badge-primary" : "badge-ghost"}`} onClick={handleOpen}><MapPin size={11} /><span data-i18n-skip>{canvases.find((canvas) => canvas.id === placement.canvasId)?.name}</span> · {placement.accessMode}</button>)}</div></td><td><div className="flex justify-end gap-1">{!placedHere && !dfdOnly && <button type="button" data-seed-id={seed.id} className="btn btn-ghost btn-xs gap-1" onClick={handlePlace}><Plus size={13} />Place here</button>}<button type="button" data-seed-id={seed.id} className="btn btn-ghost btn-xs gap-1" disabled={dfdOnly || !owner || canvases.length < 2} onClick={handleTransfer}><ArrowRightLeft size={13} />Transfer</button>{owner && <button type="button" data-canvas-id={owner.canvasId} data-seed-id={seed.id} className="btn btn-ghost btn-xs btn-square" onClick={handleOpen} aria-label={`Open owner canvas for ${seed.title}`}><ExternalLink size={13} /></button>}</div></td></tr>;
