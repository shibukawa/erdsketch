...
export function OwnershipTransferDialog({ seed, canvases, placements, onTransfer, onClose }: OwnershipTransferDialogProps) {
  ...
  return <div className="modal modal-open" role="dialog" aria-modal="true" aria-labelledby="ownership-transfer-title">
    <div className="modal-box max-w-lg rounded-xl bg-white">
      <div className="flex items-start justify-between"><div><p data-i18n-skip className="text-xs font-bold uppercase tracking-wide text-slate-500">{seed.title}</p><h2 id="ownership-transfer-title" className="text-xl font-bold">Change owner canvas</h2></div><button type="button" className="btn btn-ghost btn-sm btn-square" onClick={onClose}><X size={18} /></button></div>
      {!owner ? <div className="alert alert-warning mt-5">This model has no exclusive owner and cannot be transferred here.</div> : <form className="mt-5" onSubmit={handleSubmit}>
        <label className="form-control"><span className="label-text mb-2 font-bold">New owner canvas</span><select className="select select-bordered w-full" value={targetId} onChange={handleTargetChange}>{candidates.map((canvas) => <option data-i18n-skip key={canvas.id} value={canvas.id}>{canvas.name}</option>)}</select></label>
        <div className="mt-5 rounded-lg border border-slate-200 bg-slate-50 p-4"><p className="text-xs font-bold uppercase tracking-wide text-slate-500">Effect preview</p><div className="mt-3 flex items-center gap-2 text-sm"><span data-i18n-skip className="rounded-md bg-white px-2 py-1 font-semibold">{canvases.find((canvas) => canvas.id === owner.canvasId)?.name}</span><span className="badge badge-ghost">readonly</span><ArrowRight size={15} /><span data-i18n-skip className="rounded-md bg-white px-2 py-1 font-semibold">{canvases.find((canvas) => canvas.id === targetId)?.name}</span><span className="badge badge-primary">owner</span></div><p className="mt-3 text-xs text-slate-500">{targetPlacement ? "The existing readonly placement will become the owner." : "A placement will be created on the target canvas."}</p></div>
