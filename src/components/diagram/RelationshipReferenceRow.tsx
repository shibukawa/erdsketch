...
export function RelationshipReferenceRow({ modelId, relationship, reference, canEdit, refinementSelected, onToggleRefinement, onTogglePrimaryKey, onToggleForeignKey, onToggleVisibility, onDelete }: RelationshipReferenceRowProps) {
  ...
  return (
    <li className={`grid h-10 grid-cols-[40px_minmax(140px,1fr)_120px_78px_70px_56px_44px_40px] items-center border-b border-slate-100 text-sm ${hidden ? "bg-slate-50 text-slate-500" : "bg-blue-50/45"}`} role="row">
      ...
      <span className="flex min-w-0 items-center gap-2 truncate px-2 font-mono text-[13px] font-semibold text-blue-900"><span data-i18n-skip className="truncate">{relationship.name}</span><span className="rounded bg-white/80 px-1 py-0.5 text-[9px] uppercase text-slate-500">{relationship.kind ?? "foreign-key"}</span></span>
      <span className="px-2 text-center text-[10px] font-bold text-slate-300">—</span>
