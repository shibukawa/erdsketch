...
function PreviewCard({ seed, original }: { seed: ModelSeed; original?: ModelSeed }) {
  ...
  return <article className="min-w-[190px] rounded-xl border border-slate-200 bg-white p-3 shadow-sm">
    ...
    <h4 data-i18n-skip className="mt-1 font-bold text-slate-900">{seed.title}</h4>
    ...
    <ul className="mt-2 space-y-1">{seed.fields.map((field) => {
      ...
      return <li key={field.id} className={`flex items-center justify-between gap-2 rounded px-2 py-1 font-mono text-[11px] ${inherited ? "border border-dashed border-violet-200 bg-violet-50 text-violet-800" : added.has(field.id) ? "bg-emerald-50 text-emerald-800" : "bg-slate-50 text-slate-700"}`}><span data-i18n-skip>{field.primaryKey ? "🔑 " : ""}{field.name}</span><span className="shrink-0 font-sans text-[8px] font-bold uppercase tracking-wide opacity-70">{inherited ? "Inherited" : "Owned"}</span></li>;
    })}</ul>
