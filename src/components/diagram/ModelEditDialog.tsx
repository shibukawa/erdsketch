...
import { useEffect, useRef, useState, useTransition, type MouseEvent, type PointerEvent, type SyntheticEvent, type WheelEvent } from "react";
...
type ModelEditDialogProps = {
  ...
  domains: DataDomain[];
  canEdit: boolean;
  onSave: (patch: Partial<ModelSeed>) => Promise<boolean>;
  onClose: () => void;
};
...
export function ModelEditDialog({ model, domains, canEdit, onSave, onClose }: ModelEditDialogProps) {
  ...
  function handleDraftChange(patch: Partial<ModelSeed>) { setDraft((current) => ({ ...current, ...patch })); }
  function handleVolumeChange(volumeEstimate: NonNullable<ModelSeed["volumeEstimate"]>) { handleDraftChange({ volumeEstimate }); }
  function handleSqlChange(additionalSql: string) { handleDraftChange({ additionalSql }); }
  function handleTabClick(event: MouseEvent<HTMLButtonElement>) { setTab(event.currentTarget.dataset.tab as ModelEditTab); }
  function handleSubmit(event: SyntheticEvent<HTMLFormElement>) {
    event.preventDefault();
    startSaveTransition(async () => {
      await onSave({ names: draft.names, vocabularyBinding: draft.vocabularyBinding, description: draft.description, role: draft.role, dependency: draft.dependency, hasPrivacy: draft.hasPrivacy, volumeEstimate: draft.volumeEstimate, additionalSql: draft.additionalSql });
    });
  }
  function handleCancel(event: SyntheticEvent<HTMLDialogElement>) { event.preventDefault(); if (!saving) onClose(); }
  function handleBackdropClick(event: MouseEvent<HTMLDialogElement>) { if (!saving && event.target === event.currentTarget) onClose(); }
  function stopPointer(event: PointerEvent<HTMLDialogElement>) { event.stopPropagation(); }
  function stopWheel(event: WheelEvent<HTMLDialogElement>) { event.stopPropagation(); }
    ...
    <dialog ref={dialogRef} className="field-list-dialog m-auto h-[min(90vh,860px)] w-[min(96vw,1180px)] rounded-xl border border-slate-200 bg-white p-0 text-slate-950 shadow-2xl" aria-labelledby="model-edit-title" onCancel={handleCancel} onClick={handleBackdropClick} onPointerDown={stopPointer} onWheel={stopWheel}>
      <form className="flex h-full min-h-0 flex-col" onSubmit={handleSubmit}>
        <header className="shrink-0 border-b border-slate-200 px-5 pt-4">
          <div className="flex items-start justify-between gap-5"><div><p className="text-xs font-bold uppercase tracking-wide text-blue-600">Model definition</p><h2 id="model-edit-title" className="mt-1 text-xl font-bold">Edit {draft.names?.business ?? draft.title}</h2></div><button type="button" className="btn btn-ghost btn-sm btn-square -mr-1 -mt-1" onClick={onClose} aria-label="Close model editor" disabled={saving}><X size={18}/></button></div>
        ...
        <footer className="flex shrink-0 justify-end gap-2 border-t border-slate-200 px-5 py-4"><button type="button" className="btn btn-ghost" onClick={onClose} disabled={saving}>Cancel</button><button type="submit" className="btn btn-primary" disabled={!canEdit || saving}>{saving ? "Saving…" : "Save model"}</button></footer>
      </form>
    </dialog>,
    document.body
  );
