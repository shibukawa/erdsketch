...
export function VocabularyRegistrationDialog({ match, entries, onCreateEntry, onBindingChange, onClose }: VocabularyRegistrationDialogProps) {
  ...
  return <div role="dialog" aria-modal="true" aria-labelledby="vocabulary-registration-title" className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/35 p-4"><form className="w-[min(92vw,720px)] rounded-xl border border-slate-200 bg-white shadow-2xl" onSubmit={handleSubmit}><header className="border-b border-slate-200 px-5 py-4"><h3 id="vocabulary-registration-title" className="text-lg font-bold">Register unmatched vocabulary</h3><p className="text-sm text-slate-500">Insert <code>|</code> where each dictionary entry should be split.</p></header><div className="p-5"><p className="mb-2 text-sm"><strong>Source:</strong> {match.sourceText}</p><input autoFocus className="input input-bordered w-full text-lg" value={segmentation} onChange={handleChange} aria-label="Vocabulary segmentation" /><div className="mt-4 flex flex-wrap gap-2">{segments.map((segment, index) => <button type="button" key={`${segment}-${index}`} data-index={index} className={`badge badge-lg ${selectedIndexes.has(index) ? "badge-primary" : "badge-outline"}`} aria-pressed={selectedIndexes.has(index)} onClick={handleSegmentToggle}>{segment}</button>)}</div><p className="mt-3 text-xs text-slate-500">Selected segments will be registered. Click a segment to leave it unmatched.</p><p className="mt-1 text-xs text-slate-500">Remove all separators to register the whole phrase. Add separators freely for Japanese, Chinese, Korean, or multi-word groups.</p></div><footer className="flex justify-end gap-2 border-t border-slate-200 px-5 py-4"><button type="button" className="btn btn-ghost" onClick={onClose}>Cancel</button><button type="submit" className="btn btn-primary" disabled={saving || selectedIndexes.size === 0}>{saving ? "Registering…" : "Register selected"}</button></footer></form></div>;
}
