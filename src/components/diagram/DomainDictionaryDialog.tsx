...
function createComponent(name: string) {
  return { id: crypto.randomUUID(), name, required: true, description: "" };
}
...
export function DomainDictionaryDialog({ domains, categories, canEdit, assignmentTarget, onChange, onCreateDomain, onCreateCategory, onChangeCategory, onImportCategory, onDelete, onAssign, onClose }: DomainDictionaryDialogProps) {
  ...
  const handleKindChange = useCallback((event: ChangeEvent<HTMLSelectElement>) => {
    ...
    if (shape === "unresolved") {
      updateSelected({ shape, primitiveType: undefined, bits: undefined, length: undefined, precision: undefined, scale: undefined, components: [] });
      return;
    }
    if (shape === "composite") {
      updateSelected({ shape, primitiveType: undefined, bits: undefined, length: undefined, precision: undefined, scale: undefined, components: selectedDomain?.components ?? [] });
      return;
    }
      ...
      <div className="flex h-full min-h-0 flex-col">
        ...
        <div className="grid min-h-0 flex-1 grid-cols-[190px_minmax(300px,0.9fr)_minmax(420px,1.2fr)]">
          <nav className="flex min-h-0 flex-col border-r border-slate-200 bg-slate-50 p-3" aria-label="Domain categories">
            ...
            <label className="mt-3 block"><span className="mb-1 block text-[10px] font-bold uppercase tracking-wider text-slate-400">Add category</span><input className="input input-bordered input-sm w-full bg-white" value={categoryQuickEntry} onChange={handleCategoryQuickEntryChange} onKeyDown={handleCategoryQuickEntryKeyDown} disabled={!canEdit} placeholder="Name + Enter" aria-label="New domain category" /></label>
          </nav>
          <section className="flex min-h-0 flex-col border-r border-slate-200" aria-label="Domain list">
            <div className="space-y-2 border-b border-slate-200 p-4"><input className="input input-bordered h-10 w-full bg-slate-50 text-sm" value={query} onChange={handleSearchChange} placeholder="Search domains" aria-label="Search domains" />{selectedCategory?.system ? <p className="px-1 text-xs text-slate-500">Built-in generic domains are fixed.</p> : <input className="input input-bordered h-10 w-full bg-white text-sm" value={domainQuickEntry} onChange={handleDomainQuickEntryChange} onKeyDown={handleDomainQuickEntryKeyDown} disabled={!canEdit} placeholder="Add domain name + Enter" aria-label="New dictionary domain name" />}</div>
                ...
                <div className="space-y-3">
                  ...
                  {selectedDomain.components.map((component, index) => (
                    <div key={component.id} data-component-id={component.id} className={`space-y-2 rounded-md border bg-white p-3 ${componentDropTargetId === component.id ? "border-blue-500 ring-2 ring-blue-200" : "border-slate-200"} ${draggingComponentId === component.id ? "opacity-40" : ""}`} onDragOver={handleComponentDragOver} onDrop={handleComponentDrop}>
                      ...
                      <div className="flex items-center gap-3"><input key={`${component.id}:${component.description ?? ""}`} className="input input-bordered input-sm min-w-0 flex-1 bg-white text-xs" data-component-id={component.id} defaultValue={component.description ?? ""} onBlur={handleComponentDescriptionCommit} onKeyDown={handleComponentInputKeyDown} disabled={!canEdit} placeholder="Description" aria-label={`${component.name} description`} /><label className="flex shrink-0 cursor-pointer items-center gap-2 text-xs font-semibold text-slate-600"><input type="checkbox" className="checkbox checkbox-sm" data-component-id={component.id} checked={component.required} onChange={handleComponentRequiredChange} disabled={!canEdit} />Required</label></div>
                    </div>
                  ))}
