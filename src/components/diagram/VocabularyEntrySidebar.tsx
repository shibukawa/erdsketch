...
export function VocabularyEntrySidebar({ entry, cache, language, onCommit, onDelete, onClose }: VocabularyEntrySidebarProps) {
  ...
  return <aside className="flex w-[min(38vw,430px)] shrink-0 flex-col border-l border-slate-200 bg-slate-50/70" aria-label={`Details for ${entry.businessName}`}>
    ...
    <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-5">
      ...
      <label className="block text-xs font-bold text-slate-600">Notes<textarea key={`${entry.id}:memo`} data-i18n-skip data-entry-id={entry.id} data-entry-key="memo" className="textarea textarea-bordered mt-1 min-h-24 w-full bg-white font-normal" defaultValue={entry.memo} onBlur={onCommit} /></label>
