...
export function CapacityEstimatePanel({ model, domains, canEdit, onChange }: CapacityEstimatePanelProps) {
  ...
  const missingFields = useMemo(() => [...new Set(projections.flatMap((projection) => projection.missingFieldNames))], [projections]);
  ...
  return (
    <div className="grid min-h-full grid-cols-[320px_1fr]">
      ...
      <section className="p-5">
        ...
        {missingFields.length > 0 && <div className="mt-5 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-950"><p className="font-bold">Incomplete field-size estimate</p><p className="mt-1 text-xs">Set estimated average size for: {missingFields.join(", ")}. Unknown sizes are not silently counted as zero; totals above are incomplete.</p></div>}
      </section>
    </div>
  );
}
