    ...
    <div className="modal-box max-w-xl rounded-xl bg-white p-0 shadow-2xl">
      ...
      <div className="space-y-5 p-6">
        ...
        <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
          <button type="button" className="btn btn-ghost gap-2" onClick={onCloseWorkspace}><X size={16} />Close</button>
          {hasSnapshot && <button type="button" className="btn btn-primary gap-2" onClick={onViewSnapshot}><Eye size={16} />View read-only snapshot</button>}
        </div>
      </div>
    </div>
  </div>;
