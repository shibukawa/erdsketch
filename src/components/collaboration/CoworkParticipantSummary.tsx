...
export function CoworkParticipantSummary({ me, users, connected, isHost, onOpenCowork, onLeaveSession, iconSize = 16 }: CoworkParticipantSummaryProps) {
  ...
  return <div className="group relative flex items-center">
    {isHost && <button type="button" className="btn btn-primary btn-sm gap-2" onClick={onOpenCowork} aria-describedby="cowork-participant-list"><Share2 size={iconSize} />{translateText("Co-work", locale)}</button>}
    <button type="button" className={`flex h-9 items-center rounded-full px-1.5 outline-none transition hover:bg-slate-100 focus-visible:ring-2 focus-visible:ring-violet-500 ${isHost ? "ml-1" : ""}`} aria-label={translateText(`Show all ${participants.length} Co-work participants`, locale)} aria-describedby="cowork-participant-list">
    ...
    <div className="invisible absolute right-0 top-full z-50 w-72 pt-2 opacity-0 transition duration-150 group-hover:visible group-hover:opacity-100 group-focus-within:visible group-focus-within:opacity-100">
      <div id="cowork-participant-list" className="max-h-80 overflow-y-auto rounded-xl border border-slate-200 bg-white p-2 shadow-xl" role="tooltip">
        <div className="flex items-center justify-between px-2 py-1.5"><p className="text-xs font-bold uppercase tracking-wide text-slate-500">{translateText("Co-work participants", locale)}</p><span className="rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-bold text-slate-600">{participants.length}</span></div>
        <ul className="mt-1 space-y-1">
          {participants.map((user) => {
            ...
            return <li key={user.id} className="flex items-center gap-3 rounded-lg px-2 py-2 hover:bg-slate-50">
              ...
              <span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold text-slate-800"><span data-i18n-skip>{user.name}</span>{isMe ? ` (${translateText("You", locale)})` : ""}</span><span className="flex items-center gap-1.5 text-xs text-slate-500"><span className={`h-1.5 w-1.5 rounded-full ${online ? "bg-emerald-500" : "bg-slate-300"}`} />{translateText(online ? "Connected" : "Disconnected", locale)}</span></span>
              {isMe && isHost && <span className="rounded-full bg-violet-50 px-2 py-1 text-[10px] font-bold text-violet-700">{translateText("Host", locale)}</span>}
            </li>;
          })}
        </ul>
        {onLeaveSession && <button type="button" className="btn btn-ghost btn-sm mt-2 w-full justify-start gap-2 text-amber-700" onClick={onLeaveSession}><LogOut size={15} />{translateText("Leave this tab session", locale)}</button>}
      </div>
    </div>
  </div>;
}
