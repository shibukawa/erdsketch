import { AlignLeft, Cable, Group, KeyRound, Search, TriangleAlert } from "lucide-react";
import { useCallback, type MouseEvent } from "react";
import type { CardDisplayMode, DfdFlow, DfdGroup, DfdNode, ModelSeed } from "../../features/modeling/types";
...
export function DfdSidebar({ selectedNode, selectedGroup, selectedFlow, selectedModel, warnings, nodes, groups, models, displayMode, externalDefinitions, onDisplayModeChange, onQuickCreate, onOpenModelPicker, onUpdateNode, onUpdateFlow, onUpdateModel, onUngroup, onDeleteSelected, onFocusWarning }: Props) {
  const handleDisplayMode = useCallback((event: MouseEvent<HTMLButtonElement>) => onDisplayModeChange(event.currentTarget.dataset.mode as CardDisplayMode), [onDisplayModeChange]);
  return <aside className="z-20 flex w-[330px] shrink-0 flex-col overflow-y-auto border-r border-slate-200 bg-white px-5 py-5 shadow-sm"><div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-lg bg-blue-700 text-white"><Cable size={20} /></div><div><p className="text-sm font-semibold text-slate-500">ERDSketch</p><h1 className="text-xl font-bold">Data Flow</h1></div></div>
    <section className="mt-6"><p className="text-xs font-bold uppercase tracking-wide text-slate-500">Quick create</p><DfdQuickCreate onCreate={onQuickCreate} /></section>
    <section className="mt-4"><p className="text-xs font-bold uppercase tracking-wide text-slate-500">Card content</p><div className="mt-2 grid grid-cols-2 rounded-lg bg-slate-100 p-1" role="group" aria-label="DFD card content"><button type="button" data-mode="description" className={`flex items-center justify-center gap-2 rounded-md px-2 py-2 text-xs font-bold ${displayMode === "description" ? "bg-white text-slate-950 shadow-sm" : "text-slate-500"}`} onClick={handleDisplayMode}><AlignLeft size={14} />Description</button><button type="button" data-mode="key-fields" className={`flex items-center justify-center gap-2 rounded-md px-2 py-2 text-xs font-bold ${displayMode === "key-fields" ? "bg-white text-slate-950 shadow-sm" : "text-slate-500"}`} onClick={handleDisplayMode}><KeyRound size={14} />Details</button></div></section>
    ...
    <div className="mt-auto pt-6"><button className="btn btn-outline w-full justify-start gap-2" onClick={onOpenModelPicker}><Search size={16} />Search project models</button><p className="mt-3 flex items-center gap-2 text-[11px] text-slate-500"><Group size={13} />Overlap same-class nodes to group them.</p></div>
  </aside>;
}
