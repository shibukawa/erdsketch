...
function RoughFlow({ route, selected, bidirectional }: { route: FlowRoute; selected: boolean; bidirectional: boolean }) {
  ...
  useEffect(() => {
    ...
    const options = { roughness: 1, bowing: 1, stroke: selected ? "#2563eb" : "#334155", strokeWidth: selected ? 3 : 2 };
...
export function DfdFlowLayer({ flows, nodes, groups, selectedFlowId, onSelect }: Props) {
  ...
  return <>{flows.map((flow) => {
    ...
    return <div key={flow.id} data-dfd-flow={flow.id} className="pointer-events-none absolute inset-0">
      <RoughFlow route={route} selected={flow.id === selectedFlowId} bidirectional={Boolean(flow.bidirectional)} />
