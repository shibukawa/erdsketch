...
export function DfdQuickCreate({ onCreate }: DfdQuickCreateProps) {
  ...
  return <form onSubmit={handleSubmit} className="mt-2 space-y-2">
    <div role="radiogroup" aria-label="DFD item type" className="grid grid-cols-3 gap-1.5">{options.map((option) => <label key={option.kind} className={`btn btn-xs min-w-0 whitespace-nowrap px-1 text-[11px] ${kind === option.kind ? "btn-primary" : "btn-outline"}`}><input type="radio" className="sr-only" name="dfd-kind" value={option.kind} checked={kind === option.kind} onChange={handleKindChange} />{option.label}</label>)}</div>
